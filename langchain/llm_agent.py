...
def provide_context(action: NeedsContext):
    if action.needs_clickables_context:
        clickables_context = get_clickables_context()
        return f"Html of the buttons/links from the page: {clickables_context}. Respond to the following natural language prompt: {prompt}"
    elif action.needs_inputs_context:
        inputs_context = get_inputs_context()
        return f"Html of the text inputs from the page: {inputs_context}. Respond to the following natural language prompt: {prompt}"
    elif action.needs_tab_context:
        tab_context = get_tab_context()
        return f"Ordered tab url list from the browser: {tab_context}. Respond to the following natural language prompt: {prompt}"
...
def do_browser_action(input: str):
    response = without_context_agent.invoke({"input": input})
    print(response.tool_calls)

    # if isinstance(response, NeedsContext):
    #     print("Agent needs context")
    #     context_prompt = provide_context(response)
    #     response = with_context_agent_executor.invoke(f"{convince}: {prompt}")
